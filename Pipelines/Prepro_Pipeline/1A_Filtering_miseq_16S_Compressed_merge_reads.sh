...
export DATA="$1"
echo $DATA
# Set the directory PATH of results"_Q_"$2"_O_"  change it accordingly ...
RESULTS=$WORK/"results_Q_"$2"_O_"$3
...
ls *_R1_001.fastq.gz > $DATAWORK/All_16s_R1_samples_prefix.txt

cd $DATAWORK

cat All_16s_R1_samples_prefix.txt > All_16s_samples_prefix.txt

sed -i 's/_R1_001.fastq.gz//g' All_16s_samples_prefix.txt
...
do
	...
	$SICKLE pe -f "$DATA/$i"_R1_001.fastq.gz -r "$DATA/$i"_R2_001.fastq.gz -t $tech -o "$DATAWORK/$TRIM/$i"_R1.trim.fastq -p "$DATAWORK/$TRIM/$i"_R2.trim.fastq -s "$DATAWORK/$SINGLE/$i".singles.fastq -q $QT -l 100
